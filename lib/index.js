...
module.exports = function setupDevEnv({
    ...
    const copyAllChaincode = () => Promise.all(chaincodeLocations.map(location => copyChaincode(location, chaincodeDestination)));
                            ...
                            upgradePromise = upgradePromise.then(() => {
                                ...
                                return copyChaincode(location, chaincodeDestination).then(() => {
                                    ...
                                    return upgradeChaincode(location, version)
                                        .then(() => console.log(`Chaincode for ${location} upgraded successful to version ${version}`))
                                }).catch(err => {
